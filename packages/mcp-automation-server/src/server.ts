  ...
  private registerTools(): void {
    ...
    this.server.tool(
      ...
      )?.description || "",
      {
        ...
        siteAutomationConsent: z
          .boolean()
          .describe(
            "Explicit acknowledgment that automating this job site's UI (login/apply " +
              "forms) may violate its Terms of Service. Must be true — required in " +
              "addition to (not instead of) the HITL approval token.",
          ),
        approverIdentity: z
          .string()
          .optional()
    ...
    this.server.tool(
      ...
      automationServerCapabilities.find(
        (c) => c.name === "request_support_action_approval",
      )?.description || "",
      {
        ticketId: z.string().describe("Support ticket ID"),
        actionType: z
          .string()
          .describe("Type of action (e.g. refund, password_reset)"),
                ...
                text: JSON.stringify(
                  {
                    ok: true,
                    approvalToken: token,
                    message: "Action staged for approval.",
                  },
                  null,
                  2,
                ),
    ...
    this.server.tool(
      "record_support_resolution",
      automationServerCapabilities.find(
        (c) => c.name === "record_support_resolution",
      )?.description || "",
      {
      ...
      async ({
        ...
        try {
          ...
          if (!isValid)
            throw new Error(
              "Execution Blocked: Invalid, expired, or tampered approval token.",
            );
            ...
            content: [
              {
                ...
                text: JSON.stringify(
                  {
                    ok: true,
                    message: `Support action ${actionType} for ticket ${ticketId} resolved.`,
                  },
                  null,
                  2,
                ),
            ...
            content: [
              {
                type: "text",
                text: JSON.stringify(
                  { ok: true, message: `Mock result for ${st}` },
                  null,
                  2,
                ),
              },
