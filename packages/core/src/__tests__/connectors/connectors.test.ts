...
describe('Source Connectors', () => {
  ...
  it('OKFDirectoryConnector should ingest .md files', async () => {
    ...
    const items = await connector.ingest({ type: 'okf-directory', path: tmpDir });
  ...
  it('MarkdownDirectoryConnector should ingest generic markdown', async () => {
    ...
    const items = await connector.ingest({ type: 'markdown-directory', path: tmpDir });
    expect(items).toHaveLength(1);
  ...
  it('OpenApiConnector should extract endpoints', async () => {
    ...
    const listUsersItem = items.find(i => i.metadata.conceptId === 'listUsers');
