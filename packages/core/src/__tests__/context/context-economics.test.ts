...
describe('Context Economics', () => {
  ...
  describe('RelevanceScore', () => {
    ...
    it('should calculate score based on task match', () => {
      const highRelevance = RelevanceScore.calculate(mockDoc, 'software engineer resume');
      const lowRelevance = RelevanceScore.calculate(mockDoc, 'unrelated marketing task');

      expect(highRelevance).toBeGreaterThan(lowRelevance);
    });

    it('should apply priority boosts', () => {
      const normalDoc = { ...mockDoc, frontmatter: { ...mockDoc.frontmatter, priority: undefined } };
  ...
  describe('ContextPlanner', () => {
    const docs: OKFDocument[] = [
      {
        conceptId: 'doc-1',
        frontmatter: { title: 'High Priority', priority: 'critical', type: 'guide' },
        body: 'Short body',
        filePath: '/1.md'
      },
      {
    ...
    it('should exclude documents that exceed the budget', () => {
      ...
      expect(manifest.documentsExcluded.some(d => d.id === 'doc-2')).toBe(true);
