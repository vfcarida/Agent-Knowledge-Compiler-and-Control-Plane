...
describe("OpenWikiDocsTarget", () => {
  ...
  it("should preserve frontmatter and emit metadata json", async () => {
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(path.resolve(process.cwd(), outDir), {
      recursive: true,
    });
    ...
    const fileCall = writeFileCalls.find((call) =>
      call[0].toString().includes("skills-test.md"),
    );
    ...
    const metaCall = writeFileCalls.find((call) =>
      call[0].toString().includes(".akcp-openwiki-metadata.json"),
    );
