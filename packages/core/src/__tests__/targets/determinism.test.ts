...
const mockIR: AgentKnowledgeIR = {
  ...
  links: [
    { sourceConceptId: 'concept1', targetConceptId: 'concept2', relationType: 'relates_to' }
  ]
};
...
describe('Compile Targets Determinism', () => {
  ...
  const testTargetDeterminism = async (TargetClass: any, outName: string) => {
    ...
    const out1 = await target1.compile(mockIR, { type: target1.targetType, out: path.join(tmpDir1, outName) });
    const out2 = await target2.compile(mockIR, { type: target2.targetType, out: path.join(tmpDir2, outName) });

    expect(out1.hash).toBe(out2.hash);
    expect(out1.bytesWritten).toBe(out2.bytesWritten);
    // ensure output exists
    const exists = await fs.access(out1.outputPath).then(() => true).catch(() => false);
    expect(exists).toBe(true);
  };

  it('IrJsonTarget should be deterministic', () => testTargetDeterminism(IrJsonTarget, 'ir.json'));
  it('OkfBundleTarget should be deterministic', () => testTargetDeterminism(OkfBundleTarget, 'bundle.okf'));
  it('OpenWikiDocsTarget should be deterministic', () => testTargetDeterminism(OpenWikiDocsTarget, 'wiki'));
  it('AgentsMdTarget should be deterministic', () => testTargetDeterminism(AgentsMdTarget, 'agents-snippet.md'));
  it('McpResourcesManifestTarget should be deterministic', () => testTargetDeterminism(McpResourcesManifestTarget, 'manifest.json'));
  it('PolicyBundleTarget should be deterministic', () => testTargetDeterminism(PolicyBundleTarget, 'policies.json'));
  it('EvalDatasetTarget should be deterministic', () => testTargetDeterminism(EvalDatasetTarget, 'eval.jsonl'));
  it('GraphJsonTarget should be deterministic', () => testTargetDeterminism(GraphJsonTarget, 'graph.json'));
});
