...
function createMockFileSystemAdapter(): IFileSystemAdapter {
  return {
    ...
    writeFile: vi.fn<(filePath: string, content: string) => Promise<void>>().mockResolvedValue(undefined),
    exists: vi.fn<(filePath: string) => Promise<boolean>>(),
    mkdir: vi.fn<(dirPath: string) => Promise<void>>().mockResolvedValue(undefined),
    deleteFile: vi.fn<(filePath: string) => Promise<void>>().mockResolvedValue(undefined),
    listFiles: vi.fn<(dirPath: string, pattern?: string) => Promise<string[]>>(),
  };
}
...
describe('OKFFileRepository', () => {
  ...
  it('should save document with valid frontmatter', async () => {
    ...
    const [writtenPath, writtenContent] = vi.mocked(mockFs.writeFile).mock.calls[0];
