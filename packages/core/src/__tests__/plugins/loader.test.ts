...
import { PluginLoader, PluginSecurityError, PluginValidationError } from '../../plugins/loader.js';
import * as path from 'path';

describe('PluginLoader', () => {
  it('throws PluginValidationError if manifest is missing', async () => {
    const fakeDir = path.resolve(__dirname, 'non-existent');
    expect(() => PluginLoader.loadManifest(fakeDir)).toThrow(PluginValidationError);
  });
  ...
  it('throws PluginSecurityError if plugin lacks required permissions', async () => {
    ...
    const exampleDir = path.resolve(__dirname, '../../../../../plugins/example-markdown-connector');
  ...
  it('loads successfully if permissions match', async () => {
    ...
    try {
      ...
      const { manifest } = await PluginLoader.loadPlugin(exampleDir, ['fs:read']);
