...
describe('Lifecycle Freshness', () => {
  ...
  it('should respect explicit status override', () => {
    expect(Freshness.getEffectiveStatus({ status: 'deprecated' })).toBe('deprecated');
    expect(Freshness.getEffectiveStatus({ status: 'archived', lastReviewedAt: '2000-01-01', reviewCadenceDays: 10 })).toBe('archived');
  });
