...
export function calculateScorecard(ir: AgentKnowledgeIR, rawFiles: { path: string, content: string }[] = []): ScorecardReport {
  ...
  if (ir.concepts.length > 0) {
    const rootFiles = ir.concepts.filter(c => !c.source.filePath.includes('/') && !c.source.filePath.includes('\\'));
  ...
  dimensions.push({
    ...
    maxScore: 10,
    details: [`Found ${ir.concepts.length} concepts, with ${structureScore}0% in subdirectories.`]
  });

  // 2. OKF Compatibility (10 pts)
  let okfScore = 0;
  if (ir.concepts.length > 0) {
    const validOkf = ir.concepts.filter(c => c.frontmatter && c.frontmatter.type);
  ...
  let economyScore = 0;
  if (ir.concepts.length > 0) {
    const totalTokens = ir.concepts.reduce((sum, c) => sum + (c.budget?.estimatedTokens || 0), 0);
  ...
  let mcpScore = 0;
  const hasMcpTarget = ir.targets?.some((t: any) => t === 'mcp-profile-server' || t === 'mcp-automation-server' || t?.type?.includes('mcp'));
  if (hasMcpTarget) mcpScore = 10;
  dimensions.push({
    dimension: ScorecardDimension.MCPReadiness,
    score: mcpScore,
    maxScore: 10,
    details: hasMcpTarget ? ['MCP target detected in compile configuration.'] : ['No MCP targets detected.']
  });
  ...
  dimensions.push({
    ...
    score: policyScore,
    maxScore: 10,
    details: policyScore === 10 ? ['Policies object is populated.'] : ['No policies detected.']
  });

  // 6. Security Posture (10 pts)
  let securityScore = 0;
  if (ir.policies?.defaultAutonomyLevel || ir.policies?.piiHandling || ir.policies?.disableDangerousTools !== undefined) {
    securityScore = 10;
  }
  dimensions.push({
    ...
    score: securityScore,
    maxScore: 10,
    details: securityScore === 10 ? ['Security policy constraints found.'] : ['Missing explicit security constraints.']
  });

  // 7. Provenance (10 pts)
  let provenanceScore = 0;
  if (ir.concepts.length > 0 && ir.concepts.some(c => c.provenance?.sourceHash)) {
    provenanceScore = 10;
  }
  dimensions.push({
    ...
    maxScore: 10,
    details: provenanceScore === 10 ? ['Provenance hashes generated.'] : ['Provenance generation is disabled.']
  });

  // 8. Evals (10 pts)
  let evalsScore = 0;
  const hasEvalTargets = ir.targets?.some((t: any) => t === 'eval-dataset' || t?.type === 'eval-dataset');
  const hasTestFiles = rawFiles.some(f => f.path.includes('.test.ts') || f.path.includes('__tests__') || f.path.includes('evals/'));
  ...
  dimensions.push({
    ...
    score: evalsScore,
    maxScore: 10,
    details: evalsScore === 10 ? ['Evaluation configurations/files found.'] : ['No evaluation integration detected.']
  });
  ...
  dimensions.push({
    ...
    maxScore: 10,
    details: [`Bundle has ${ir.concepts.filter(c => c.isStale).length} stale concepts.`]
  });

  // 10. DX (10 pts)
  let dxScore = 0;
  const hasIndex = rawFiles.some(f => f.path === 'index.md' || f.path === 'index.yaml');
