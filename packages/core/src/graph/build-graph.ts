...
export function buildSemanticGraph(ir: AgentKnowledgeIR) {
  ...
  edges.forEach(e => {
    if (outgoingCount[e.sourceConceptId] !== undefined) {
      outgoingCount[e.sourceConceptId] = (outgoingCount[e.sourceConceptId] || 0) + 1;
    }
    if (!e.isBroken && incomingCount[e.targetConceptId] !== undefined) {
      incomingCount[e.targetConceptId] = (incomingCount[e.targetConceptId] || 0) + 1;
    }
  });
  ...
  const highlyConnectedConcepts = Array.from(validConceptIds)
    ...
    .sort((a, b) => ((incomingCount[b] || 0) + (outgoingCount[b] || 0)) - ((incomingCount[a] || 0) + (outgoingCount[a] || 0)));
