...
export class OpenWikiConnector implements KnowledgeSourceConnector {
  ...
  async ingest(config: ConnectorConfig): Promise<RawKnowledgeItem[]> {
    if (!config.path) {
      throw new Error(`OpenWikiConnector requires a 'path' config pointing to the openwiki directory.`);
    }
    ...
    for (const relativePath of files) {
      ...
      items.push({
        ...
        metadata: {
          ...
          isIndex: path.basename(relativePath).toLowerCase() === 'readme.md' || path.basename(relativePath).toLowerCase() === 'index.md'
        },
        rawContent: content
      });
    }
