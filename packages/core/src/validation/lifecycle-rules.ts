  ...
  public static validate(ir: AgentKnowledgeIR): void {
    ...
    for (const concept of ir.concepts) {
      if (concept.status === 'stale') {
        console.warn(`[Lifecycle] WARNING: Concept '${concept.conceptId}' is marked as STALE. Consider reviewing it.`);
      } else if (concept.status === 'deprecated') {
        const successor = concept.frontmatter.successor;
        if (successor) {
          console.warn(`[Lifecycle] WARNING: Concept '${concept.conceptId}' is DEPRECATED. Successor: '${successor}'.`);
        } else {
          console.warn(`[Lifecycle] WARNING: Concept '${concept.conceptId}' is DEPRECATED but has no successor defined.`);
        }
      }
    }
      ...
      for (const link of ir.links) {
        ...
        if (target) {
          if (target.status === 'stale') {
            console.warn(`[Lifecycle] WARNING: '${link.sourceConceptId}' depends on STALE concept '${link.targetConceptId}'.`);
          } else if (target.status === 'deprecated') {
            console.warn(`[Lifecycle] WARNING: '${link.sourceConceptId}' depends on DEPRECATED concept '${link.targetConceptId}'.`);
          }
        }
      }
    }
