...
export class ContextPlanner {
  public static plan(documents: OKFDocument[], options: ContextPlanOptions): ContextPackManifest {
    ...
    for (const item of scoredDocs) {
      if (manifest.totalEstimatedTokens + item.estimatedTokens > manifest.budgetTokens) {
      ...
      } else if (options.budget.maxDocuments && manifest.documentsIncluded.length >= options.budget.maxDocuments) {
        manifest.documentsExcluded.push({
          id: item.doc.conceptId,
          title: item.doc.frontmatter.title || item.doc.conceptId,
          relevance: item.relevance,
