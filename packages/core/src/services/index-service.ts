...
import type { IFileSystemAdapter, IFrontmatterParser, IIndexService } from '../domain/interfaces.js';
...
export class IndexService implements IIndexService {
  ...
  async generate(directoryPath: string): Promise<string> {
    ...
    const entries: Array<{ title: string; type: string; description: string; fileName: string }> = [];
    ...
    for (const entry of entries) {
      ...
      lines.push(`| [${escapedTitle}](./${entry.fileName}) | ${escapedType} | ${escapedDesc} |`);
    }
