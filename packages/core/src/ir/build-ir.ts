...
export async function buildKnowledgeIR(bundlePath: string, options: BuildOptions = {}): Promise<AgentKnowledgeIR> {
  ...
  for (const sourceConfig of sourcesToIngest) {
    ...
    switch (sourceConfig.type) {
      ...
      case 'markdown-directory':
        items = await new MarkdownDirectoryConnector(fsAdapter).ingest(sourceConfig);
        break;
      case 'openwiki':
      ...
      case 'openapi':
        ...
        break;
      default:
        console.warn(`[WARN] Unknown source type '${sourceConfig.type}', skipping.`);
    }
  ...
  const concepts: IRConcept[] = allRawItems.map(item => {
    ...
    if (previousIr && previousIr.sourceHashes && previousIr.sourceHashes[item.sourceUri] === item.contentHash) {
      // Find the concept in previous IR
      const prevConcept = previousIr.concepts.find(c => 
  ...
  if (skippedCount > 0) {
    console.log(`[INFO] Incremental build: reused ${skippedCount} unchanged concepts from cache.`);
  }
  ...
  concepts.forEach(concept => {
    ...
    if (concept.body) {
      ...
      extracted.forEach(e => {
        ...
        const exists = links.find(l => l.sourceConceptId === concept.conceptId && l.targetConceptId === e.targetConceptId && l.relationType === e.relationType);
        if (!exists) {
          links.push({
            sourceConceptId: concept.conceptId,
