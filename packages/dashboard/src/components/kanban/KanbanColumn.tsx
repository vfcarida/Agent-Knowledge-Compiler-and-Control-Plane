...
export function KanbanColumn({ title, status, applications }: KanbanColumnProps) {
  ...
  const getHeaderStyles = (colStatus: string) => {
    switch (colStatus.toLowerCase()) {
      case 'saved': return 'border-l-zinc-500 text-zinc-400';
      case 'applied': return 'border-l-neon-indigo text-indigo-400';
      case 'screening': return 'border-l-sky-400 text-sky-400';
      case 'interview': return 'border-l-neon-blue text-cyan-400';
      case 'offer': return 'border-l-emerald-400 text-emerald-400';
      case 'rejected': return 'border-l-rose-500 text-rose-400';
      case 'withdrawn': return 'border-l-orange-500 text-orange-400';
      default: return 'border-l-zinc-700 text-zinc-500';
    }
  };
  ...
  const getHeaderDot = (colStatus: string) => {
    switch (colStatus.toLowerCase()) {
      ...
      case 'applied': return 'bg-neon-indigo';
      case 'screening': return 'bg-sky-400';
      case 'interview': return 'bg-neon-blue';
  ...
  return (
    <div className="flex-1 min-w-[300px] flex flex-col h-full bg-zinc-950/20 rounded-2xl border border-dark-border/40 p-4">
      ...
      <div className={`flex items-center justify-between border-l-2 pl-3 ${getHeaderStyles(status)} mb-5`}>
