 ...
 */
export function parseOKFContent(content: string, fileName: string, category: string): OKFDoc {
...
export function parseLogEntries(content: string): Array<{
  ...
  const entries: Array<{ timestamp: string; action: string; conceptId: string; details?: string }> = [];
  let inTable = false;
