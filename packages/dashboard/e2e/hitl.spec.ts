...
test.describe("Human-In-The-Loop (HITL) Approval Flow", () => {
  ...
  test("should display pending approval in the queue and authorize it", async ({
    page,
  }) => {
    // Navigate to the Dashboard Welcome Screen
    await expect(
      page.locator("h2").filter({ hasText: "Welcome" }),
    ).toBeVisible();
    ...
    await expect(
      page
        .locator("h2")
        .filter({ hasText: "Human-in-the-Loop Approval Queue" }),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=https://example.com/job/123"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Are you sure you want to authorize this operation?"),
    ).toBeVisible();
    ...
    await expect(
      page.locator(
        "text=Action successfully approved and submitted to the agent.",
      ),
    ).toBeVisible();
  });
});
