...
_Expected Output: Bundle is marked as valid._
...
_Expected Output: A `dist/` directory is created with `context-pack.json` and a static OpenWiki._
...
_Expected Output: The server runs and exposes tools defined in `capabilities/capabilities.json` governed by `policies/career.policy.yaml`._
