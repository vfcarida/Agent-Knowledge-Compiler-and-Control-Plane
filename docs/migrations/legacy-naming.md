...
- **Open Career Format Orchestrator (OCF Orchestrator)**: The initial thesis of the project was heavily focused on the _Career_ domain, orchestrating resumes, skill sets, and job data (which is now just one domain flagship example).
...
1. **The Career Domain Flagship**: The directory `examples/domains/career/` and `sample-data/.okf/` contain examples specific to the Career domain. Terms like "career data management" are allowed _only_ when specifically describing this domain data, not the orchestrator itself.
...
3. **Legacy CLI Shim (Historical)**: The file `packages/cli/src/legacy-ocf-bin.ts` previously existed to provide a deprecation warning and backward compatibility for the old `ocf` binary alias; this shim has since been removed in v0.1.0.
