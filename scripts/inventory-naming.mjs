  ...
  for (const line of lines) {
    for (const term of legacyTerms) {
      if (line.toLowerCase().includes(term.toLowerCase())) {
        ...
        if (
          filePath.endsWith("build-state.json") &&
          line.includes(rootDir.replace(/\\/g, "/"))
        )
          continue;
        if (filePath.endsWith("build-state.json") && line.includes(rootDir))
          continue;
...
fs.writeFileSync(
  path.join(rootDir, "inventory.json"),
